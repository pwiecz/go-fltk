...
cmake -G Ninja -S fltk -B build -DFLTK_BUILD_TEST=OFF -DCMAKE_BUILD_TYPE=Release -DFLTK_BUILD_EXAMPLES=OFF -DFLTK_BUILD_FLUID=OFF -DFLTK_BUILD_FLTK_OPTIONS=OFF -DOPTION_USE_SYSTEM_LIBJPEG=OFF -DOPTION_USE_SYSTEM_LIBPNG=OFF -DOPTION_USE_SYSTEM_ZLIB=OFF -DCMAKE_INSTALL_PREFIX=%CMAKE_INSTALL_PREFIX% -DCMAKE_INSTALL_INCLUDEDIR=%CMAKE_INSTALL_INCLUDEDIR% -DFLTK_INCLUDEDIR=%CMAKE_INSTALL_PREFIX%/%CMAKE_INSTALL_INCLUDEDIR% -DCMAKE_INSTALL_LIBDIR=%CMAKE_INSTALL_LIBDIR% -DFLTK_LIBDIR=%CMAKE_INSTALL_PREFIX%/%CMAKE_INSTALL_LIBDIR%
IF %ERRORLEVEL% NEQ 0 EXIT /B 1
